...
namespace Rotina
{
    public partial class F_edicao : Form
    {
        ...
        private void Menu_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            else if (e.ClickedItem.Name == "tsmi_mostrar")
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (clb_editar.CheckedItems.Count != 0)
            {
                string check = " ", checkeds;
                foreach (string c in clb_editar.CheckedItems)
                {
                    check += c + ", ";
                }
                checkeds = check.TrimEnd(", ".ToCharArray());
                MessageBox.Show(checkeds);
            }
